...
pub fn start(eapp_dir: &Path, db_file: &Path) -> Child {
  let mut geoserver_home = eapp_dir.to_path_buf();
  geoserver_home.push("geoserver");
...
--module=webapp
  ...
  )).expect("Unable to write geoserver start.ini");
...
<web-app>
  ...
  "#
  ).expect("Unable to write geoserver/WEB-INF/web.xml");
...
"#).expect("Unable to write to geoserver/resources/log4j.properties");
...
:end
  ...
  "#).expect("Unable to write to geoserver/bins/startup.bat");
...
exec "$_RUNJAVA" $JAVA_OPTS $MARLIN_ENABLER -DGEOSERVER_DATA_DIR="$GEOSERVER_DATA_DIR" -Djava.awt.headless=true -DSTOP.PORT=8079 -DSTOP.KEY=geoserver -jar start.jar --dry-run
  ...
  "#).expect("Unable to write to geoserver/bins/startup.sh");
  ...
  #[cfg(not(windows))]
  {
    ...
    let r = Command::new("chmod")
          ...
          .output();
    if let Err(e) = r {
      println!("Error chmod +x geoserver startup.sh: {}", e);
    }
  }
  ...
  if let Some(split) = shlex::split(&second_line[..]) {
    ...
    return Command::new(&split[0])
        ...
        .env(crate::LOCI_DB_ENV_KEY, &db_file.to_string_lossy()[..]) // Geoserver doesn't use this, but this is the general idea.
        .args(&split[1..])
        ...
        .spawn()
        .expect("geoserver command failed to start");
